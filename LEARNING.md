...
- **Middleware chaining for auth**: `router.get("/me", protect, me)` runs `protect` first; it only reaches `me` by calling `next()`. `protect` reads the `Authorization: Bearer <token>` header, verifies it with `jwt.verify()`, and attaches the decoded user to `req.user`. `authorize(...roles)` is a middleware *factory* — it returns a middleware function checking `req.user.role`, used for RBAC (e.g. admin-only routes). It must run after `protect`, since it depends on `req.user` existing.
- **API keys vs passwords**: passwords use bcrypt (deliberately slow, to resist guessing a low-entropy human password). API keys use a fast hash (SHA-256) instead, because the key itself is already random/high-entropy — hashing is just to stop a stolen database from leaking usable keys, and a fast hash allows instant lookup-by-hash on every ingestion request, which matters for performance.
- **Show-once secrets pattern**: the raw API key is only ever returned in the response to *creating* an application. Only its hash and a short preview (last 4 chars) are stored — the same pattern GitHub/Stripe use for API tokens.
- **Embedded vs referenced documents**: `Trace.spans` is an array embedded directly inside the Trace document, not a separate collection, because spans are always read together with their trace (the waterfall view needs all of them at once) — this avoids an extra query/join. This is a MongoDB-specific pattern; a relational DB would normalize this into a separate table.
- **`Mixed` schema type**: span `attributes` uses `mongoose.Schema.Types.Mixed` (an open-ended field) because different span types (retrieval vs. LLM call) genuinely have different attributes — this is the concrete reason the project chose MongoDB over a rigid relational schema.
- **Compound index for the dashboard query**: `traceSchema.index({ application: 1, createdAt: -1 })` pre-organizes data for the exact query the dashboard runs constantly (an application's traces, most recent first) — without it, that query gets slower as trace volume grows.
- **Async evaluation as nullable fields**: `evaluation.score`/`rationale` default to `null` since evaluation happens after ingestion via a queue, not during it — a freshly ingested trace has telemetry but no evaluation yet.
